...
function parseBudgetRange(budgetStr) {
  if (!budgetStr) return [0, 999999];

  // ✅ 새 형식: "3~8" (단위: 억)
  if (budgetStr.includes('~')) {
    const [minStr, maxStr] = budgetStr.split('~').map(s => s.trim());
    const minEok = Number(minStr);
    const maxEok = Number(maxStr);
    if (!isNaN(minEok) && !isNaN(maxEok) && minEok > 0 && maxEok > 0 && minEok <= maxEok) {
      // 백엔드 쪽은 만원 단위로 쓰고 있었으니 그대로 맞춤
      return [minEok * 10000, maxEok * 10000];
    }
  }
  const patterns = {
    '3억 이하': [0, 30000],
